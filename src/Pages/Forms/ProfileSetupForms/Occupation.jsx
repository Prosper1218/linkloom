...
const FNandLN = () => {
   ...
   const {setoccupation} = useOutletContext();
   const occupationref = useRef();
   const handlesubmit = (e) => {
      e.preventDefault();
      const occ = occupationref.current.Job.value;
      setoccupation(occ);
      if (occ) {
         setTimeout(() => {
            navigate("/setup/Dob");
      ...
      <div className="min-h-[100vh]" style={{backgroundColor: theme === "dark" ? "#222222" : "#F7F7F8"}}>
         ...
         <div className="flex justify-center px-3">
            <div className=" w-[500px]">
               <form className="grid gap-0 sm:gap-3 grid-cols-1 sora" ref={occupationref}>
                  <input
                     type="text"
                     name="Job"
                     id="Job"
                     className={`w-full h-[2.5rem] rounded-[4px] focus:outline-none bg-transparent pl-4 text-xs mb-3`}
                     style={{color: theme === "dark" ? "#F7F7F8" : "#222222", border: theme === "dark" ? "0.5px solid #F7F7F8 " : "0.5px solid #222222 "}}
                     placeholder="Occupation..."
                     required
                  />
               </form>
               <button className="float-right h-[2.5rem] rounded-[4px] montserrat  w-24 bg-[#480057] capitalize font-serif text-white text-xs" onClick={handlesubmit}>
                  Next
               </button>
            </div>
         </div>
