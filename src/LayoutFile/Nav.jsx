...
const Nav = () => {
   ...
   return (
      <div>
         ...
         <aside
            id="default-sidebar"
            className={`fixed top-0 left-0 z-40 w-[8rem] h-screen translate-x-[-13rem] sm:translate-x-0 transition ease-in-out delay-150 bg-[#490057]
         ${NavActive ? "translate-x-[0px]" : ""}`}
            aria-label="Sidebar"
         >
            <div className="h-full overflow-y-auto bg-rgba(73, 0, 87, 1)-800 ">
               <div className=" flex justify-end w-full">
                  <button className="inline-flex items-center p-2 ms-3 text-sm sm:hidden focus:outline-none bg-white rounded-sm" type="button">
                     <Cross1Icon className="float-right" onClick={() => setNavActive(false)} />
                  </button>
               </div>
